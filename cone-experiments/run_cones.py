...
def evaluate_single_trial(args):
    ...
    query_order = fwd_merge_queries(widths_list, offsets_list, D, strategy_seed, oracle_seed)
...
def warmup():
    ...
    _estimate_from_layer0(np.zeros(4), 4)
    oracle_query_single(42, 0, 2)
    # Run one trial to compile everything
    evaluate_single_trial(([4, 4], [0, 4], 2, 42, 42))

def run_D(D, n_samples, n_workers):
    w = funnel(D)
    ol = [sum(w[:i]) for i in range(D)]
    k = D

    rng = np.random.default_rng(42)
    args = [(w, ol, D, int(rng.integers(0, 2**62)), int(rng.integers(0, 2**62)))
...
if __name__ == "__main__":
    ...
    print(f"Cores: {n_workers}, Time budget: {args.minutes} min")
    ...
    for D in D_values:
        ...
        t0 = time.monotonic()
        for _ in range(3):
            evaluate_single_trial((w, ol, D, 12345, 42))
        ...
        t0 = time.monotonic()
        mean, stderr, trials = run_D(D, n, n_workers)
